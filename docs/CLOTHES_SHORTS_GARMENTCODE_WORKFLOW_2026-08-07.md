...
## Proxy 闭合截面校准探针（v47，2026-08-07）

- 新增 `tools/garmentcode/calibrate_body_measurements_from_proxy.py`，使用 `trimesh` 的闭合水平截面直接测量代理，而不是用稀疏 OBJ 顶点估算；腰线、臀线取整圈，腿线默认将连接截面除以 2。
- v47 使用 v9 proxy 直接生成 Actor 专用 body YAML，测得腰/臀/单腿围约为 `167.67/160.46/83.71 cm`，并用原 v2 的 Pants 参数重新生成纸样。
- 60 步短程：身体交叉 `163`、自交 `34`；120 步：身体交叉 `493`、自交 `0`。300 步版本在 240 秒内只完成 187 步并超时。日志分别为 `third_party/GarmentCode/Logs/actor_v1_shorts_proxy_calibrated_seed_2_260807-16-48-53/`、`...-16-53-30/`、`...-16-54-57/`。
- 结论：直接把 proxy 的总截面围长写回 GarmentCode body 会使纸样和计算规模显著膨胀，120 步已劣于 v9 的 300 步基线，不能进入 Gallery。此前的稀疏顶点测量工具也不能作为自动校准依据。
- 下一步：保留该工具作为诊断，先把左右大腿截面从代理中分离并校准，再单独处理腰/臀的 Actor 外轮廓；不要再把合并的双腿截面直接当作单个 GarmentCode body 参数。
