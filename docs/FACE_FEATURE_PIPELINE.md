...
可以实现。当前眼睛主要是带透明 PNG 的眼部面片，并不是完整的面部骨骼系统，因此最稳妥的方案不是给头部强行增加复杂面部骨骼，而是准备三种眼睛状态：打开、半闭、闭合，然后在 3D 渲染或导出帧阶段按时间采样切换贴图/UV。若未来需要更自然的闭眼曲线，再增加上眼睑遮罩的 Shape Key 或独立遮挡面片。该方案能直接兼容四向渲染、像素化和随机五官，但应在当前走路与耳朵绑定稳定后单独制作眨眼测试，不放入本轮动作基线。
