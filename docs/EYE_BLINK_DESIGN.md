...
眨眼对大头角色的表情收益高，且只需要切换眼睛图层；当前采用挂在 `CC_Base_Head` 的浅曲面眼睛层，先在 Blender 中渲染，再进入项目的 2D/像素化输出。背面没有眼睛几何，因此背面保持透明，不需要生成背面眼睛贴图。

## 推荐实现

- 3D 渲染阶段准备 `open`、`half`、`closed` 三种状态；第一版视觉状态先使用 image_gen 生成的 `open` 与 `closed` 贴图，`half` 只保留在时间表合同中，待眼睑形变方案确认后再启用。
...
第一阶段使用 3D 相机自动覆盖正面、右侧、左侧和背面验证；背面不生成眼睛，保持透明 Face 层。侧面是否可读由相机和眼睛曲面自然决定，不另造一套运行时方向贴图。
...
当前不建议先给 Blender 角色增加复杂面部骨骼或真实眼睑变形。第一版使用独立眼睛曲面和 image_gen 状态贴图切换；如果需要 `half`，再单独验证 Shape Key，不让它影响已稳定的头部绑定。

## 当前实现

- Blender 构建脚本：`tools/blender/build_eye_blink_experiment.py`
- 无窗口材质渲染：`tools/blender/render_eye_blink_experiment.py`
- image_gen 闭眼源的去色键与尺寸归一化：`tools/process_imagegen_eye_texture.py`
- 派生实验输出：`prototype/test_output/eye_anime/`（不修改 Actor V1 原始 Blend）
- 当前实验状态：`open/closed` 已通过 3D 材质切换验证；`half` 仍是下一步形变实验，不作为本次通过条件。
