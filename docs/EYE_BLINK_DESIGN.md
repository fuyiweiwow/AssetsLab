...
- 资源基准：最终 EyeAssembly 应优先使用 `prototype/assets/characters/actor_v1/eye_textures/eye_right.png` 与 `eye_left.png`；当前 image_gen 资产仅用于验证 3D 结构，不能作为 Actor 比例基准。
- 当前 gallery 测试候选已切换为 Actor 原生 `eye_left.png` / `eye_right.png`，image_gen 资产不再作为当前渲染输入。
