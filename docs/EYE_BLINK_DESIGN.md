...
- 当前实验状态：v12 改为独立 `body` / `eyes` / `composite` 三阶段；身体层不含眼睛，眼睛层为透明 RGBA，合成后再进入 gallery。前、左、右、背四向均已通过检查，背面无眼睛。
- Gallery 参考：`prototype/preview/animation_gallery/eye-anime-v12/`；其中 64px GIF 仅为最近邻观察，不是最终像素资产。
...
- v12 独立层入口：`tools/blender/render_eye_blink_experiment.py --layer body|eyes`，合成入口：`tools/composite_eye_layers.py`。
