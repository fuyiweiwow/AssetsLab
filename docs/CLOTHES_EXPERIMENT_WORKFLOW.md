...
无袖上衣已形成当前里程碑；短袖外挂、裤子和盔甲尚未解决。后续应以 current 覆盖方式逐个增加复杂部件，
每次只推进一个隔离阶段。
...
## 12. 肩带骨骼基准与下摆穿模诊断（2026-08-06）

本轮复核确认之前的流程虽然读取了 Actor 肩部横向宽度，但没有把肩带的垂直上缘、
肩端横向位置和肩部表面深度同时纳入版型约束，导致“看似用了骨骼数据，肩带仍没有
实际高过肩膀”。第 1 帧基准记录如下：左右上臂骨头部约为 `z=1.385/1.398 m`，
锁骨尾部平均约为 `z=1.391 m`；肩带顶边必须高于该连接区，并且外边不能停在旧的
`0.68` 横向采样点。

本轮还确认了两个容易重复的错误：

- Actor 输入网格在肩部高度包含头部顶点，且少量头部顶点继承了脊柱权重；直接取
  `min/max y` 会把衣片前深度推到脸部前方，形成侧面悬空横条。流程现在要求先按
  躯干/锁骨/上臂骨骼权重过滤，再在 `x/z` 局部截面内剔除深度离群点；
- 下摆顶点如果继续继承髋部不对称权重，会在走路帧被大腿拉开，形成半圆形缺口。当前
  Render Garment 的下摆环统一使用 `CC_Base_Waist 0.65 + CC_Base_Spine01 0.35`，
  并将版型下缘提升到腰骨尾端以上，作为穿模门禁的一部分。

当前脚本已经记录 `landmarks`、`hem_weight_stabilization` 和躯干采样方法到
`garmentcode_proxy_render_pair_current/manifest.json`。后续审核必须分别检查：
肩带是否跨过肩骨、侧面是否仍有悬空横条、下摆是否被大腿穿出；不能只看正面静止帧。

## 11. 无袖背心袖窿与肩带回归验证（2026-08-06）

第 10 节候选虽然消除了背部水平条带，但视觉上又退化成“正面有 U 型口的布裹在身上”。
复核发现这是版型拓扑问题，不是材质或像素化问题：前片领口开口被扩大到
`0.34/0.50/0.60`，顶部肩带只剩很窄的边；同时肩部连接列使用了不对称的
`0/1/4`，且侧缝一直封到肩顶，因而没有真实的袖窿。

另一次扩大上胸宽度并把开口扩大到 `0.74` 的试验已废弃。该值超过相邻横向采样点
`0.68`，造成行内点序不单调，不能作为有效版型参数。扩大肩宽本身可以保留，但必须和
受约束的领口一起使用。

当前 Gallery 候选只做以下最小修复：

- 上部半宽恢复到 Actor 肩部包络：`0.33/0.35/0.38/0.365`；
- 前领口收回为 `0.26/0.40/0.48`，给左右肩带留下连续材料；
- 前后片顶部只连接对称肩带列 `1/3`；
- 上部外侧列 `0/4` 删除为袖窿，侧缝只连接到腋下行；
- 继续使用 `Animation Proxy` 的 63 组权重和同一个 Actor Armature，不改变运动绑定。

该候选已完成 4 方向 × 8 帧后台渲染，并覆盖 Gallery current；状态仍为
`review_required`，需先通过正面肩带、侧面袖窿和背面完整性审核，才允许进入 milestone。
