...
- 当前实验参数：`sleeve-length-fraction=0.90`、`sleeve-clearance=0.012m`、袖筒半径范围 `0.10--0.18m`；
- 袖筒在 `armature_rest_pose` 中生成，并使用 `forward_offset=0.04m` 与 `lateral_offset=0.10m` 解决 Actor 放松姿态下手臂位于躯干后侧、袖筒被躯干轮廓遮挡的问题；左右符号按本 Actor 的 L=正 X、R=负 X 处理；
...
人工检查发现首版袖筒只在侧面露出残留，正面像卡进手臂。复核单独袖筒渲染后确认不是材质丢失，而是几个几何问题叠加：袖筒之前在姿态空间生成后又被 Armature modifier 变形一次，半径采样上限过小，中心线落在躯干正面轮廓内，且左右外扩符号曾经写反。现已改为 REST 空间生成、对应上臂权重采样，袖长提高到上臂的 `0.90`，修正 L/R 符号并加入适度外扩。四方向动作帧重新渲染，自动适配门禁为 `pass`；当前版本正面已能辨识连续袖片，仍需人工确认是否需要进一步改为官方前后袖片连接结构。

## 检测器边界修正（2026-08-07）

肩部定位只属于确认里程碑的衣身，独立袖筒的外扩距离不应被当成衣身肩部悬浮。检测器现将 shoulder placement 限定为躯干网格，同时继续对合并后的衣身和袖筒执行身体间隙、穿透、下摆、边界和非流形检查；肩袖交界的合法重叠区从 `|y|<0.08m` 放宽到 `|y|<0.12m`，避免 256 px 审核分辨率下的边界误报。
