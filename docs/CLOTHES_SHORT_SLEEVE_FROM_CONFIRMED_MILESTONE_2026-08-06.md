...
- 当前实验参数：`sleeve-length-fraction=0.90`、`sleeve-clearance=0.012m`；
- 袖筒在 `armature_rest_pose` 中生成，并使用 `forward_offset=0.04m` 与 `lateral_offset=0.07m` 解决 Actor 放松姿态下手臂位于躯干后侧、袖筒被躯干轮廓遮挡的问题；
...
人工检查发现首版袖筒只在侧面露出残留，正面像卡进手臂。复核单独袖筒渲染后确认不是材质丢失，而是几个几何问题叠加：袖筒之前在姿态空间生成后又被 Armature modifier 变形一次，半径采样上限过小，且袖筒中心线落在躯干正面轮廓内。现已改为 REST 空间生成、对应上臂权重采样，袖长提高到上臂的 `0.90`，并加入前向及左右外扩偏移。四方向动作帧重新渲染，自动适配门禁仍为 `pass`；人工审核仍需确认正面是否已形成连续短袖轮廓。
