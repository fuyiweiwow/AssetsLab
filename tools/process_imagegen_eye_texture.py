...
def fit_texture(image: Image.Image, size: tuple[int, int], max_height_fraction: float) -> Image.Image:
...
def main() -> int:
    ...
    base = fit_texture(Image.open(options.source), (options.width, options.height), options.max_height_fraction)
