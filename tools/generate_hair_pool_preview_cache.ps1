param(
    ...
    [string]$Gender = "all",
    [string]$OutputRoot = "prototype\test_output\hair_candidates_2026_08_04\pool_preview_gallery"
)
...
try {
    ...
    $pool = (Get-Content -Raw "prototype\assets\hair\hair_random_pool_v1.json" | ConvertFrom-Json).components
    ...
    foreach ($genderName in $genders) {
        ...
        if ($genderName -eq "female") {
            ...
            foreach ($b in $base) { foreach ($f in $front) { foreach ($s in $side) {
                Invoke-HairPreview $genderName @($b.object, $f.object, $s.object) "prototype\assets\hair\Blender-Chloe_Hair.blend" "Chloe_head_dummy" $true
            }}}
        } else {
            $back = @($items | Where-Object { $_.role -eq "back_section" })
            foreach ($b in $base) { foreach ($s in $side) {
                Invoke-HairPreview $genderName @($b.object, $s.object) "prototype\assets\hair\male_source\Blend_Hair.blend" "Colin_head_dummy" $false
                foreach ($k in $back) {
                    Invoke-HairPreview $genderName @($b.object, $s.object, $k.object) "prototype\assets\hair\male_source\Blend_Hair.blend" "Colin_head_dummy" $false
                }
            }}
        }
    }
