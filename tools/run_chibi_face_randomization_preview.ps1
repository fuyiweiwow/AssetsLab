param(
    [string]$BlenderPath,
    [string]$PythonPath,
    [string]$InputBlend = "prototype\assets\characters\generated\chibi_eyes_ears_pixel_walk_source_v1.blend",
    [string]$Output = "prototype\test_output\face_randomization_v2",
...
$defaultBlender = Join-Path (Split-Path $root -Parent) "blender-4.5.10-windows-x64\blender.exe"
...
$testRoot = [System.IO.Path]::GetFullPath((Join-Path $root "prototype\test_output"))
if (-not $outputPath.StartsWith($testRoot + [System.IO.Path]::DirectorySeparatorChar, [System.StringComparison]::OrdinalIgnoreCase)) {
    throw "Output must remain under prototype\\test_output: $outputPath"
}
