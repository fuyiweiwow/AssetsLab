...
const MODEL := preload("res://scripts/calibrated_head_attachment_stage.gd")
const OUTPUT_DIRECTORY := "res://test_output/skeleton_pipeline/calibrated_head_attachment"
...
func _run() -> void:
	...
	await process_frame
	var errors := model.validate_head_attachment()
	...
	for index in range(8):
		...
		if image.save_png(directory_path.path_join("frame_%02d.png" % index)) != OK:
			...
			return
	print("CALIBRATED_HEAD_ATTACHMENT_STAGE_PASS frames=8 directions=4 head_neck=locked output=%s" % directory_path)
	quit(0)


func _fail(message: String) -> void:
	push_error("CALIBRATED_HEAD_ATTACHMENT_STAGE_FAIL: " + message)
	quit(1)
