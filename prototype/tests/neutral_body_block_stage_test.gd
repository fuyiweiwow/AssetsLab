...
const MODEL := preload("res://scripts/neutral_body_block_stage.gd")
const OUTPUT_DIRECTORY := "res://test_output/skeleton_pipeline/neutral_body_blocks"
...
func _run() -> void:
	...
	await process_frame
	var errors := model.validate_body_blocks()
	...
	for index in range(8):
		...
		if image.save_png(directory_path.path_join("frame_%02d.png" % index)) != OK:
			...
			return
	print("NEUTRAL_BODY_BLOCK_STAGE_PASS frames=8 directions=4 mannequin=geometric output=%s" % directory_path)
	quit(0)


func _fail(message: String) -> void:
	push_error("NEUTRAL_BODY_BLOCK_STAGE_FAIL: " + message)
	quit(1)
