...
const MODEL := preload("res://scripts/back_arm_swing_stage.gd")
const OUTPUT_DIRECTORY := "res://test_output/skeleton_pipeline/back_arm_swing"
...
func _run() -> void:
	...
	await process_frame
	var errors := model.validate_back_arm_swing()
	...
	for index in range(8):
		...
		if image.save_png(directory_path.path_join("frame_%02d.png" % index)) != OK:
			...
			return
	print("BACK_ARM_SWING_STAGE_PASS frames=8 lower_body=back_pelvis_unchanged arms=counterphase output=%s" % directory_path)
	quit(0)


func _fail(message: String) -> void:
	push_error("BACK_ARM_SWING_STAGE_FAIL: " + message)
	quit(1)
