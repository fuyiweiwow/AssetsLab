...
const MODEL := preload("res://scripts/left_mirror_stage.gd")
const OUTPUT_DIRECTORY := "res://test_output/skeleton_pipeline/left_mirror"
...
func _run() -> void:
	...
	await process_frame
	var errors := model.validate_left_mirror()
	...
	for index in range(8):
		...
		if image.save_png(directory_path.path_join("frame_%02d.png" % index)) != OK:
			...
			return
	print("LEFT_MIRROR_STAGE_PASS frames=8 source=side_arm_swing exact_joint_reflection output=%s" % directory_path)
	quit(0)


func _fail(message: String) -> void:
	push_error("LEFT_MIRROR_STAGE_FAIL: " + message)
	quit(1)
