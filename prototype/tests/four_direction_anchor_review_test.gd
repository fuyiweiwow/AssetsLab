...
const MODEL := preload("res://scripts/four_direction_anchor_review.gd")
const OUTPUT_DIRECTORY := "res://test_output/skeleton_pipeline/four_direction_anchor_review"
...
func _run() -> void:
	...
	await process_frame
	var errors := model.validate_anchor_review()
	...
	for index in range(8):
		...
		if image.save_png(directory_path.path_join("frame_%02d.png" % index)) != OK:
			...
			return
	print("FOUR_DIRECTION_ANCHOR_REVIEW_PASS frames=8 head_neck=shared pelvis=registered baseline=470 output=%s" % directory_path)
	quit(0)


func _fail(message: String) -> void:
	push_error("FOUR_DIRECTION_ANCHOR_REVIEW_FAIL: " + message)
	quit(1)
